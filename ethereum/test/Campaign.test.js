...
beforeEach(async () => {
  ...
  campaign = await new web3.eth.Contract(Lottery.abi)
    .deploy({
...
it('allows one account to enter', async () => {
  ...
  const players = await campaign.methods
    .getPlayers()
...
it('requires a minimum amount of ether to enter', async () => {
  try {
    await campaign.methods.enter().send({ from: accounts[0], value: 0 });
...
it('only manager can call pickWinner', async () => {
  try {
    await campaign.methods.pickWinner().send({ from: accounts[1] });
...
it('sends money to winner and resets the players array', async () => {
  ...
  await campaign.methods.pickWinner().send({ from: accounts[0] });
